...
// Выбока записей (posts) с автором в админку
...
function selectTopTopicsFromPostsOnIndex($table1){
    ...
    $sql = "
    SELECT * FROM $table1 WHERE id_topic = 9";
    $query = $pdo->prepare($sql);
    $query->execute();
    dbCheckError($query);
    return $query->fetchAll();
...
function seacrhInTitleAndContent($text ,$table1, $table2){
    $text = trim(strip_tags(stripcslashes(htmlspecialchars($text))));
    global $pdo;
    $sql = "
    SELECT p.*, u.username FROM $table1 AS p
    JOIN $table2 AS u
    ON p.id_user = u.id
    WHERE p.status =1
